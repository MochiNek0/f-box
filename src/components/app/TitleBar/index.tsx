...
export const TitleBar: React.FC<TitleBarProps> = ({ onSettingsClick }) => {
  ...
  const isZaoMeng = activeTab?.title?.includes("造梦西游");
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-gr-3">
        ...
        {/* Speed Hack Controls (造梦西游 only) */}
        {isZaoMeng && (
          <div className="flex items-center gap-gr-1 px-gr-1 py-0.5 bg-white/5 rounded-full border border-white/10 no-drag ml-gr-2 shadow-sm" style={noDragStyle}>
            {/* Status Icon */}
            <div className={`p-1 rounded-full transition-colors ${isSpeedActive ? "text-primary bg-primary/10" : "text-zinc-500 bg-white/5"}`}>
            ...
            <div className="relative flex items-center h-4">
              <select
                value={Math.abs(speed - Math.round(speed)) < 0.01 ? Math.round(speed) : ""}
              ...
              >
                ...
                <option value="1" className="bg-zinc-800 text-zinc-200">1x</option>
